  ...
  function fetchGoals(retries = 3) {
    api.get("/metas/listar")
      ...
      .catch((error) => {
        ...
        if (retries > 1) {
          console.log(`Tentando novamente... (${retries - 1} restantes)`);
          setTimeout(() => fetchGoals(retries - 1), 10000);
        } else {
          setMsg("Erro ao carregar dados. Tente novamente mais tarde.");
        }
      });
  }
  ...
  return (
    <div className="dashboard">
      ...
      {goals ? (
        ...
        <p>
          Carregando metas{" "}
          <i className="material-icons loading-icon">autorenew</i>
        </p>
      )}
    </div>
  );
